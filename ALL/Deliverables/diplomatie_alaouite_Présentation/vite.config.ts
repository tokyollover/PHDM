...
function saveSlidesPlugin(): Plugin {
  return {
    name: "save-slides",
    apply: "serve",
    configureServer(server) {
      const slidesPath = nodePath.resolve(__dirname, "src/data/slides.ts");

      server.middlewares.use((req, res, next) => {
        if (req.method !== "POST" || req.url !== "/__save_slides") {
          return next();
        }
        ...
        req.on("end", async () => {
          try {
            ...
            const fileContent = [
              ...
              "",
              `export const slidesContent: Slide[] = ${JSON.stringify(slides, null, 2)};`,
              "",
            ].join("\n");

            await fs.writeFile(slidesPath, fileContent, "utf8");
            res.statusCode = 200;
            res.setHeader("Content-Type", "application/json");
