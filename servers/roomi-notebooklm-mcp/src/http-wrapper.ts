...
app.get('/content/sources/:id/text', async (req: Request, res: Response) => {
  try {
    ...
    const result = await toolHandlers.handleGetSourceText({
      source_id: sourceId,
      notebook_url: typeof notebook_url === 'string' ? notebook_url : undefined,
      session_id: typeof session_id === 'string' ? session_id : undefined,
    });
...
// Get source text content by name (alternative endpoint)
app.get('/content/sources/text', async (req: Request, res: Response) => {
  try {
    const { source_name, source_id, notebook_url, session_id } = req.query;
