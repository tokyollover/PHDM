...
export class ContentManager {
   ...
   *
   * This method:
   * 1. Navigates to the Sources panel
   * 2. Finds and clicks on the specified source
   * 3. Extracts all visible text content from the source viewer
   * 4. Returns the text with metadata
   *
   * @param input - Source identifier (sourceId or sourceName)
   * @returns SourceTextResult with text content or error
   */
  async getSourceText(input: SourceTextInput): Promise<SourceTextResult> {
    const { sourceId, sourceName } = input;
    ...
    log.info(`📖 Extracting text from source: ${sourceId || sourceName}`);
    ...
    try {
      ...
      if (!resolvedSourceName) {
        try {
          resolvedSourceName = await sourceElement.$eval(
            '.source-name, .title, [class*="name"], [class*="title"], label, span',
            (e) => e.textContent?.trim() || 'Unknown'
          );
        } catch {
          resolvedSourceName = sourceId || 'Unknown';
        }
      }

      log.info(`  📄 Opening source: ${resolvedSourceName}`);

      // Click on the source to open it in the viewer
      await sourceElement.click();
      await randomDelay(1000, 1500);
      ...
      if (!viewerLoaded) {
        ...
        log.info(`  🔄 Trying double-click to open source...`);
        await sourceElement.dblclick();
        await randomDelay(1500, 2000);
      }

      // Extract text content from the source viewer
      const textContent = await this.extractSourceTextContent();
      ...
      if (!textContent) {
        return {
          ...
          error: 'Could not extract text content from source viewer',
        };
      }

      const characterCount = textContent.length;
      // Estimate passage count based on paragraph breaks
      const passageCount = (textContent.match(/\n\n+/g) || []).length + 1;

      log.success(`  ✅ Extracted ${characterCount} characters from source`);
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      log.error(`❌ Failed to extract source text: ${errorMsg}`);
      return { success: false, error: errorMsg };
    }
  }

  /**
   * Wait for the source viewer panel to load
   */
  private async waitForSourceViewer(): Promise<boolean> {
    const viewerSelectors = [
      // Source viewer/reader panel
      '.source-viewer',
      '.source-reader',
      '[class*="source-viewer"]',
      '[class*="sourceViewer"]',
      ...
      '[class*="pdf-viewer"]',
      // Content area that appears when source is opened
      '[class*="source-content"]',
      '[class*="document-content"]',
      // Main content area with scrollable text
      '.source-text',
      '[class*="source-text"]',
      // Right panel that shows source details
      '[class*="detail-panel"]',
      '[class*="right-panel"] [class*="content"]',
      // Generic content areas
      '[role="article"]',
      'article.source',
    ];
    ...
    for (const selector of viewerSelectors) {
      try {
        const el = this.page.locator(selector).first();
        if (await el.isVisible({ timeout: 2000 })) {
          log.info(`  ✅ Source viewer loaded: ${selector}`);
          return true;
        }
      } catch {
        continue;
    ...
    // Wait a bit more and check for any new content panel
    await randomDelay(1000, 1500);

    // Check if any content changed in the right panel
    try {
      const rightPanel = this.page.locator('[class*="right"], [class*="detail"], [class*="viewer"]').first();
      if (await rightPanel.isVisible({ timeout: 1000 })) {
        log.info(`  ✅ Content panel detected`);
        return true;
      }
    } catch {
      // Continue
    ...
    log.warning(`  ⚠️ Source viewer not detected`);
    return false;
  }
