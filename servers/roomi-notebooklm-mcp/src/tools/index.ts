...
User: "Yes" → call remove_notebook`,
    ...
    },
    {
      ...
      description:
        'Extract the full text content from a source document in NotebookLM.\n\n' +
        'This tool opens a source document and extracts its complete text content, ' +
        'which is useful for:\n' +
        '- Further processing or analysis of source content\n' +
        '- Creating local copies of source text\n' +
        '- Feeding source content to other tools or workflows\n' +
        '- Understanding what text NotebookLM has indexed from a source\n\n' +
        'The method:\n' +
        '1. Navigates to the Sources panel\n' +
        '2. Finds and opens the specified source\n' +
        '3. Extracts all visible text from the source viewer\n' +
        '4. Returns the text with character and passage counts\n\n' +
        'Note: For large documents, the extracted text may be truncated. ' +
        'Use list_content first to see available sources.',
      inputSchema: {
        type: 'object',
        properties: {
          source_id: {
            type: 'string',
            description: 'The unique ID of the source (optional if source_name provided)',
          },
          source_name: {
            type: 'string',
            description: 'The name/title of the source (partial match supported)',
          },
          notebook_url: {
            type: 'string',
            description: 'Notebook URL. If not provided, uses the active notebook.',
...
export class ToolHandlers {
  ...
  async handleGetSourceText(args: {
    ...
    notebook_url?: string;
    session_id?: string;
  }): Promise<ToolResult<SourceTextResult>> {
    const { source_id, source_name, notebook_url, session_id } = args;

    log.info(`🔧 [TOOL] get_source_text called`);
    if (source_id) {
      log.info(`  Source ID: ${source_id}`);
    }
    if (source_name) {
    ...
    try {
      ...
      if (!source_id && !source_name) {
        return {
          success: false,
          error: 'Either source_id or source_name is required to identify the source',
        };
      }
      ...
      // Extract source text
      const result = await contentManager.getSourceText({
        sourceId: source_id,
        sourceName: source_name,
      });
      ...
      if (result.success) {
        log.success(
          `✅ [TOOL] get_source_text completed: ${result.characterCount} chars from "${result.sourceName}"`
        );
      } else {
        log.error(`❌ [TOOL] get_source_text failed: ${result.error}`);
      }
